var ctcxHelpWordJs = {
	...
	ctcxServeObj: {
		"现状人口": {
			"url": "http://whgis.wpl:8010/ServiceAdapter/MAP/%E6%9C%80%E5%B0%8F%E7%A9%BA%E9%97%B4%E5%8D%95%E5%85%832018%E5%B9%B4%E4%BA%BA%E5%8F%A3%E7%BB%9F%E8%AE%A1/7a2743f8c33f5d654e25f47851a11032",
			"layerIds": [
				0
			],
			"outFields": ["FREQUENCY"],
		...
		"现状用地": {
			"url": "http://192.168.8.41:6080/arcgis/rest/services/TDLY/GIS_FZSYS_TDLY_CXYDXZ2018/MapServer",
			"layerIds": [
				1
			],
			"outFields": ["用地代码"],
		...
		"土地信息": {
			...
			"jy": {
				"url": "http://192.168.31.199:6080/arcgis/rest/services/GKCJ/MapServer",
				"layerIds": [
					0
				],
				"outFields": ["OBJECTID"],
	...
	xzrk: function(features) {
		...
		for(let j = 0; j < features.length; j++) {
			...
			var zd = ctcxHelpWordJs.ctcxServeObj["现状人口"]["outFields"][0]
			_n += (features[j].attributes[zd] * Proportion)
		}
		ctcxHelpWordJs.op["rk"] =_n.toFixed(0)
		ctcxHelpWordJs.isOk++
	},
	xzyd: function(features) {

		var attributes_total = {}
		var attributes_arr = []
		...
		for(let j = 0; j < features.length; j++) {
			...
			var intersect_geo = ctcxHelpWordJs.mapOperator.types.geoEngine.intersect(ctcxHelpWordJs.geo, features[j].geometry);
			var intersect_mj = ctcxHelpWordJs.mapOperator.types.geoEngine.planarArea(intersect_geo, 'hectares');
			var zd = ctcxHelpWordJs.ctcxServeObj["现状用地"]["outFields"][0]
			var attr = features[j]["attributes"][zd].substring(0, 2)
			if(attributes_total[attr]) {
				attributes_total[attr] += intersect_mj
			} else {
				attributes_total[attr] = intersect_mj
			}

		}
		for(let key in attributes_total) {
			var bl = (attributes_total[key] / range) * 100
			if(bl > 10) {
		...
		for(let i = 0; i < attributes_arr.length; i++) {
			if(ctcxHelpWordJs.ydDictionary[attributes_arr[i][0]]){
				ctcxHelpWordJs.op["yd"].push(ctcxHelpWordJs.ydDictionary[attributes_arr[i][0]].text)
			}else{
				ctcxHelpWordJs.op["yd"].push("未知类")
			}
	...
	cb: function(features) {
		...
		ctcxHelpWordJs.op["td"]["cb"]=_n.toFixed(0);
		ctcxHelpWordJs.isOk++
	},
	jy: function(features) {
		var _n=0;
		var range = Math.abs(ctcxHelpWordJs.mapOperator.types.geoEngine.planarArea(ctcxHelpWordJs.geo, "hectares"));
		for(let j = 0; j < features.length; j++) {
			var primary_mj = Math.abs(ctcxHelpWordJs.mapOperator.types.geoEngine.planarArea(features[j].geometry, 'hectares'))
		...
		ctcxHelpWordJs.op["td"]["jy"]=_n.toFixed(2);
		ctcxHelpWordJs.isOk++
	},
	...
	xzjz: function(features) {
		...
		var range = 0;
		for(let j = 0; j < features.length; j++) {
			if(ytTotal[features[j].attributes[yt]]) {
				ytTotal[features[j].attributes[yt]] += features[j].attributes[mj];
			} else {
				ytTotal[features[j].attributes[yt]] = features[j].attributes[mj];
			}
			
			if(csTotal[features[j].attributes[cs]]) {
				csTotal[features[j].attributes[cs]] += features[j].attributes[mj];
			} else {
				csTotal[features[j].attributes[cs]] = features[j].attributes[mj];
			}
			
			range += features[j].attributes[mj]
		}
		ctcxHelpWordJs.op["jz"]["z"] = range
		
		for(let key in ytTotal) {
			var bl = (ytTotal[key] / range) * 100
			if(bl > 10) {
				ytAttributes_arr.push([key, bl])
			}
		}
		for(let key in csTotal) {
			var bl = (csTotal[key] / range) * 100
			if(bl > 5) {
				csAttributes_arr.push([key, bl])
			}
		}
		...
		for(let i = 0; i < ytAttributes_arr.length; i++) {
			var ydxzStr=ytAttributes_arr[i][0]
			if(ctcxHelpWordJs.jzDictionary[ydxzStr.substr(0, 3)]) {
				ctcxHelpWordJs.op["jz"]["lwz"].push(ctcxHelpWordJs.jzDictionary[ydxzStr.substr(0, 3)])
						} else if(ctcxHelpWordJs.jzDictionary[ydxzStr.substr(0, 2)]) {
							ctcxHelpWordJs.op["jz"]["lwz"].push(ctcxHelpWordJs.jzDictionary[ydxzStr.substr(0, 2)])
						} else {
							keyAttr = "不明建筑"
						}
		}
		for(let i = 0; i < csAttributes_arr.length; i++) {
			ctcxHelpWordJs.op["jz"]["cwz"].push(csAttributes_arr[i][0])
		}
		ctcxHelpWordJs.isOk++
	},

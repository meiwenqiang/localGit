...
var xieshenPage = {
			...
			function LSajax2() {
				$.ajax({
					...
					data: {
						...
						'json': JSON.stringify(xieshenPage.xsResult_ghfa),
						'XMGK': $("#divXSYJD textarea").val()
					},
					dataType: "json",
					async: false,
	...
	clashShow: function (obj, size, laySumarea, id, isGhfa) {
		for (let i = 0; i < laySumarea.length; i++) {
			...
			if (isGhfa) {
				...
				xieshenPage[obj].polygonColorGraphAdd(geo, color, [0, 255, 255, 1], 1, attributes, id)
			} else {
	...
	compareRule: function (code, layerRuleJson, pendingdlmc, standarddlmc, intersect_geo, ydflMj, end, fun, i) {
		if (layerRuleJson && layerRuleJson[pendingdlmc]) {
			...
			try {
				xieshenPage.getRuleRes(code, theRuleRes, pendingdlmc, key, intersect_geo, ydflMj, end, fun, i)
			} catch (e) {
				console.log(e);
			}
		}
	 ...
	 * @param {*} ydflMj 面积
	 */
	getRuleRes: function (code, layerRule, pendingdlmc, standarddlmc, intersect_geo, ydflMj, end, fun, i) {
		if (layerRule) {//有规则
	...
	queryReportFa: function () {
		...
		if(xieshenPage.rkxzData==undefined  ||  xieshenPage.yztData==undefined){
	...
	showDetail_fa: function (layerName) {
		...
		for (var i = 0; i < xieshenPage.faDkGeoArr.length; i++) {
			...
			xieshenPage.mapOperator2.polygonColorGraphAdd(polygon, color, [0, 0, 0, 1], 1, attributes, 'dk');
		}
		//添加范围渲染
	...
	selectDk: function (event) {
		...
		for (var i = 0; i < event.results.length; i++) {
			if (event.results[i].graphic.layer.id == "dk") {
							...
							if (xieshenPage.js == features.length) {
								...
								for (let i = 0; i < features.length; i++) {
									try {
										...
										var lxmc = attrDictionary[pendingMc] ? attrDictionary[pendingMc] : pendingMc;
										html += "<tr data-fid='" + i + "'>"
										html += "<td>" + (i + 1) + "</td>"
										html += "<td>" + lxmc + "</td>"
										html += "<td>" + dkMj + "</td>"
										...
										if (i == 0) {
											var lx = attrDictionary[xieshenPage.attributes] ? attrDictionary[xieshenPage.attributes] : xieshenPage.attributes;
											//用地代码不合并行不合并行
	...
	getHtmlTable_xfa: function (data, isHtml) {
		...
		for (var key in topObj) {
			...
			var lx = "";
			//用地类别代码转为中文
			lx = attrDictionary[key] ? attrDictionary[key] : key;
			...
			for (var i = 0; i < row; i++) {
				...
				var lxMc = "";
				//用地类别代码转为中文
				lxMc = attrDictionary[aRow['lx']] ? attrDictionary[aRow['lx']] : aRow['lx'];
				if (i == 0) {
		...
		for (var key in dealJson) {
			...
			if (rowNum > 0) {
				for (var i = 0; i < rowNum; i++) {
					...
					var lx = "";
						//用地类别代码转为中文
					lx = attrDictionary[aRow.lx] ? attrDictionary[aRow.lx] : aRow.lx;
					if (i == 0) {
